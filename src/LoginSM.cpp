...
namespace  // Concrete FSM implementation
{
    ...
    BOOST_MSM_EUML_ATTRIBUTES((attributes_ << input_line), line_attributes)

    BOOST_MSM_EUML_EVENT_WITH_ATTRIBUTES(valid_input, line_attributes)
    ...
    BOOST_MSM_EUML_STATE((no_action, Initialize
    ), Initial)
    ...
    Confirm_Email + valid_input [is_y] == Get_New_User_Password,
    Confirm_Email + valid_input [is_n] == Get_Email,
    Confirm_Email + valid_input [!is_y && !is_n] == Confirm_Email,
    ...
    Choose_Ansi + valid_input [(is_y || is_n) && no_confirm_sent] / set_ansi == Resend_Confirm_Email,
    Choose_Ansi + valid_input [(is_y || is_n) && !no_confirm_sent] / set_ansi == Show_Account_Menu,
    Choose_Ansi + valid_input [!is_y && !is_n] == Choose_Ansi,
    ...
    Show_Creation_Menu + valid_input [
    is_5 &&class_set
    ] / reset_reroll == Choose_Stats,
    ...
    Show_Creation_Menu + valid_input [
    is_d &&character_complete
    ] / save_new_character == Show_Account_Menu,
    ...
    Choose_Name + valid_input [
    name_avalailble &&name_banned
    ] / show_name_banned == Choose_Name,
    Choose_Name + valid_input [name_available && !
    name_banned &&character_complete
    ] / abort_new_name == Show_Account_Menu,
    ...
    namespace mudbase {
        ...
        std::string &LoginSMInternal::do_state_step() {
            ...
            if (line.length() == 0) {
                fsm_.process_event(::invalid_input);
            } else {
                fsm_.process_event(::valid_input(line));
            }
        }
    }
